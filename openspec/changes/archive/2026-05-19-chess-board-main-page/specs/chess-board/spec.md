...
The system SHALL call the `onMove` callback after each accepted move with the move details and resulting FEN.

#### Scenario: Move callback fired
- **WHEN** a legal move is made
- **THEN** `onMove` is called with `{ from, to, promotion?, fen }`
