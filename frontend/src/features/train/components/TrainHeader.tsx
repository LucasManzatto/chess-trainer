import { useTrainStore, getDueCount } from '../store/trainStore'

export function TrainHeader() {
  const mode = useTrainStore(s => s.mode)
  const stats = useTrainStore(s => s.stats)
  const dueRemaining = useTrainStore(getDueCount)
    ...
    <div className="flex items-center w-full px-2 gap-2 select-none">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <span className="text-white/50 text-xs">
          <span className="text-white/90 font-semibold">{stats.correct}</span>
